...
export class NewMessageService {
  ...
  // NEW: Navigation + Current User + Conversation ID
  ...
  async sendAndNavigate(messageText: string): Promise<void> {
    ...
    const activeUser = this.auth.activeUser();
    const senderId = activeUser?.uid;
    if (!senderId) {
    ...
    await this.chatContext.openConversation(otherUserId);
