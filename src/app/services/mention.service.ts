...
export class MentionService {
  getTriggerQuery(value: string, trigger: string): string | null {
  ...
  filterUsers(users: User[], value: string): User[] | null {
  ...
  filterChannels(channels: Channel[], value: string): Channel[] | null {
    const query = this.getTriggerQuery(value, '#');
    if (!query) return null;
    const q = query.toLowerCase();
  ...
  replaceTriggerWithText(value: string, trigger: string, replacement: string): string {
    const lastIndex = value.lastIndexOf(trigger);
    if (lastIndex === -1) return value;

    const before = value.slice(0, lastIndex);
    return `${before}${trigger}${replacement} `;
  }
}
